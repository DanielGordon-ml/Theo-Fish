---
slug: best-reply-graph
name: best-reply graph
semantic_type: structure
canonical_definition: ''
formal_spec: A directed graph where vertices are strategy tuples, and there is a directed
  edge from s to s' if s ≠ s' and exactly one agent i has s_i' as a best reply to
  s_{-i} and s_{-i}' = s_{-i}.
aliases: []
status: canonical
human_notes: ''
generalizes: []
generalizes_meta: []
has_component: []
has_component_meta: []
depends_on: []
depends_on_meta: []
related_to: []
related_to_meta: []
equivalent_to: []
equivalent_to_meta: []
instantiates: []
instantiates_meta: []
sourced_from:
- '[[sources/the_evolution_of_conventions|the_evolution_of_conventions]]'
sourced_from_meta:
- target: '[[sources/the_evolution_of_conventions|the_evolution_of_conventions]]'
  section: ''
  page: ''
  confidence: 0.0
---

# best-reply graph




## Provenance

| Source | Section | Formulation | Confidence |
|--------|---------|-------------|------------|
| [[sources/the_evolution_of_conventions]] |  | "" | 0.0 |
