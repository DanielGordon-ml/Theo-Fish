---
slug: gradient-estimation-operator
name: gradient-estimation-operator
semantic_type: operator
canonical_definition: ''
formal_spec: Y_n(t) = (d_n Z_n(t-M) / δ(t-M)) Σ_{m∈𝒰_n^M(t)} a_m^n(t) u_m(t-M)
aliases: []
status: canonical
human_notes: ''
generalizes: []
generalizes_meta: []
has_component: []
has_component_meta: []
depends_on: []
depends_on_meta: []
related_to: []
related_to_meta: []
equivalent_to: []
equivalent_to_meta: []
instantiates: []
instantiates_meta: []
sourced_from:
- '[[sources/cooperative_multi_player_bandit_optimization|cooperative_multi_player_bandit_optimization]]'
sourced_from_meta:
- target: '[[sources/cooperative_multi_player_bandit_optimization|cooperative_multi_player_bandit_optimization]]'
  section: ''
  page: ''
  confidence: 0.0
---

# gradient-estimation-operator




## Provenance

| Source | Section | Formulation | Confidence |
|--------|---------|-------------|------------|
| [[sources/cooperative_multi_player_bandit_optimization]] |  | "" | 0.0 |
